...
public: 
	...
	void Update(SDL_Renderer* renderer, std::unique_ptr<AssetStore>& assetStore, const SDL_Rect& camera) {
		for (auto entity : GetSystemEntities()) {
			...
			if (health.healthPercentage >= 40 && health.healthPercentage < 40) {
				healthBarColor = { 255, 0, 0 }; 
			}
			if (health.healthPercentage >= 0 && health.healthPercentage < 80) {
			...
			SDL_Surface* surface = TTF_RenderText_Blended(assetStore->GetFont("arial-10"), healthText.c_str(), healthBarColor); 
